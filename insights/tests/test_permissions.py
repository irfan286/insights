...
class TestInsightsPermissions(InsightsIntegrationTestCase):
    ...
    def test_insights_role_is_not_enforced_for_public_access(self):
        # a public dashboard is viewed as Guest. `run_doc_method` enters
        # `permission_user` only after settling that the document, the method and
        # its arguments are publicly reachable, so the role check inside that
        # context would refuse what the publisher published.
        ...
        with self.as_user("Guest"):
            ...
            with permission_user(ADMIN):
                self.assertTrue(protected_insights_call())

            # and the bypass ends with the context
            with self.assertRaises(frappe.PermissionError):
                protected_insights_call()
